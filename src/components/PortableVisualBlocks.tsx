...
// 1. LINK: SEO-Clean + Sponsored + Warna Oranye
...
// 2. TABEL MARKDOWN: Paksa Desain 3-Warna & Scroller
...
const components = {
  block: {
    normal: ({ node, children }: any) => {
      ...
      // FIX ARTIKEL LAMA: Proteksi shortcode {% %}
  ...
  types: {
    // 4. TABEL SANITY OBJECT (Handle 3 Warna)
    table: ({ value }: any) => (
    ...
    affiliateButton: ({ value }: any) => {
      ...
      return (
        <div className="btn-stack inline">
          ...
          <a href={value.url} target="_blank" rel="nofollow sponsored noopener" className={`z-btn ${ (store === 'aliexpress' || label.toLowerCase().includes('ali')) ? 'ali-bg' : 'amz-bg' }`}>
            {label.toUpperCase()}
          </a>
        </div>
      );
...
export default function PortableVisualBlocks({ value }: { value: any }) {
  ...
  return (
    <div className={`zaidly-main-wrapper ${isClient ? 'is-active' : ''}`}>
      <style dangerouslySetInnerHTML={{ __html: `
        .zaidly-main-wrapper { width: 100%; color: #000; }
        
        /* WARNA LINK & QUOTE */
        .zaidly-link, .zaidly-main-wrapper a { color: #d97706 !important; text-decoration: underline; font-weight: 800; }
        ...
        /* WARNA 1: HEADER */
        .zaidly-table-scroller th { background: #3C2F2F !important; color: #fff !important; padding: 14px 16px; text-align: left; text-transform: uppercase; font-size: 13px; letter-spacing: 0.05em; }

        /* WARNA 2 & 3: ZEBRA STRIPES */
        ...
        .zaidly-table-scroller td { padding: 12px 16px; border-bottom: 1px solid #eee; color: #4a3728; font-weight: 600; }
        ...
        .zaidly-logo-box { height: 35px; display: flex; align-items: center; margin-bottom: 6px; }
        .amz-logo-brand { color: #000; font-weight: 900; font-size: 18px; position: relative; font-family: sans-serif; }
