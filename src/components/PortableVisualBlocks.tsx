...
// 2. TABEL MARKDOWN (DIPAKSA SCROLL)
renderer.table = ({ header, body }: any) => {
  return `<div class="zaidly-table-scroller"><table><thead>${header}</thead><tbody>${body}</tbody></table></div>`;
};

// --- FUNGSI REKURSIF UNTUK MEMBONGKAR [object Object] (OBAT MANJUR) ---
const superParse = (cell: any): string => {
  if (!cell) return "";
  if (typeof cell === "string") return cell;
  if (typeof cell === "number") return String(cell);

  // Jika sel berisi array (Struktur Sanity/Turso Abang)
  if (Array.isArray(cell)) {
    return cell.map((item) => superParse(item)).join("");
  }

  // Jika sel adalah objek, drill terus sampai ketemu teks murni
...
// --- RENDERER TABEL SANITY (3-TONE COLOR & ANTI-OBJECT) ---
...
const components = {
  block: {
    normal: ({ node, children }: any) => {
      ...
      // Proteksi shortcode artikel lama agar Astro bisa render {% %}
  ...
  types: {
    ...
    image: ({ value }: any) => {
      ...
      return imageUrl ? <div className="zaidly-img-box"><img src={imageUrl} alt={value.alt || "Zaidly Gear"} loading="lazy" /></div> : null;
    },
    productReviewCard: ({ value }: any) => {
      const rating = value.itemRating || 0;
      return (
          ...
          <div className="card-body">
            <div className="stat-box">
              ...
              <div className="stars-row">
                {[1,2,3,4,5].map((n) => <span key={n} style={{ color: n <= Math.floor(rating) ? "#ff9900" : "#ddd" }}>★</span>)}
            ...
            <div className="stat-box">
              ...
              <span className="price">{value.itemPrice || "Check Price"}</span>
            </div>
  ...
  return (
    <div className={`zaidly-master-wrapper ${isClient ? "is-ready" : ""}`}>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .zaidly-table-scroller th { background: #3C2F2F !important; color: #fff !important; padding: 14px 16px; text-align: left; text-transform: uppercase; font-size: 13px; letter-spacing: 0.05em; border: none; }
        .zaidly-table-scroller td { padding: 12px 16px; border-bottom: 1px solid #f0f0f0; color: #4a3728; font-weight: 600; }
        
        .even-row { background-color: #ffffff; }
        .odd-row { background-color: #f9f7f5; }
        ...
        .amz-logo { color: #000; font-weight: 900; font-size: 18px; position: relative; height: 35px; display: flex; align-items: center; font-family: sans-serif; }
        ...
        .zaidly-card-review { margin: 3rem 0; padding: 2.2rem; border: 1.5px solid #eee; border-radius: 16px; background: #fff; box-shadow: 0 5px 15px rgba(0,0,0,0.03); }
        ...
        .zaidly-h2, .zaidly-h3 { scroll-margin-top: 120px; font-weight: 900; color: #000; margin-top: 2.5rem; }
