...
const generateId = (children) => {
  const text = Array.isArray(children) ? children.join("") : typeof children === 'string' ? children : children?.props?.children || "";
      ...
      <ReactMarkdown
        ...
        components={{
          h1: ({node, children, ...props}) => (
            <h1 id={generateId(children)} className="text-3xl md:text-4xl font-black mt-10 mb-6 text-coffee-dark uppercase italic border-b-2 border-coffee-dark/10 pb-2" {...props}>{children}</h1>
          ),
          h2: ({node, children, ...props}) => (
            <h2 id={generateId(children)} className="text-2xl font-black mt-8 mb-4 text-coffee-dark uppercase tracking-tight" {...props}>{children}</h2>
          ),
          h3: ({node, children, ...props}) => (
            <h3 id={generateId(children)} className="text-xl font-bold mt-6 mb-3 text-coffee-dark" {...props}>{children}</h3>
          ),
