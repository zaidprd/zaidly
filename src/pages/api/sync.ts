...
// 1. Definisikan variabel turso (biar error "Cannot find name 'turso'" hilang)
...
export async function getTursoPosts() {
  // Ambil semua kolom termasuk visual_content
  const result = await turso.execute("SELECT * FROM posts ORDER BY published_at DESC");
  
  // 2. Beri tipe data 'any' pada post supaya TypeScript tidak protes
  return result.rows.map((post: any) => ({
    ...
    // Tarik data visual_content (Portable Text)
